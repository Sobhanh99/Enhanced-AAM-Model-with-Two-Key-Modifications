#landmarks should be normalized
...
def PiecewiseAffineTransform(srcIm, srcPoints, dstIm, dstPoints):
	...
	#Determine which tesselation triangle contains each pixel in the shape norm image
	inTessTriangle = np.ones(dstIm.size, dtype=int) * -1
	...
	triAffines = []
	for i, tri in enumerate(tess.simplices):   #tess.vertices in original
...
def normalize_shape(individual_img_path, target_img_path, show=True, get_location=False):
	#Load source image
	srcIm = Image.open(individual_img_path)
	dstIm = Image.open(target_img_path)

	src_pts = []
	x_values, y_values, connect_from, connect_to = get_coordinates(individual_img_path)    #use arrayed date instead of path here
	for x,y in zip(x_values, y_values):
		src_pts.append((256*x,256*y ))

	dst_pts = []
	x_values, y_values, connect_from, connect_to = get_coordinates(target_img_path)    #use arrayed date instead of path here
	for x,y in zip(x_values, y_values):
		dst_pts.append((256*x,256*y )) #x*256, y*256
	...
	if not get_location:
		try:
			shape_normalized_img = (shape_normalized_img*(img_mask!=0))[xmin:xmax, ymin:ymax]   #I changed 0 to 1
		except:
...
if __name__ == "__main__":
	normalize_shape(individual_img_path="...\\Images and Annotations\\6.jpg",
		  	target_img_path="...\\Images and Annotations\\13.jpg",show=True)
